...
trait JmsEnvelopeHeader {
  ...
  val redeliveryHeader: String => String = s => jmsHeaderPrefix(s) + "Redelivery"
...
object JmsFlowSupport extends JmsEnvelopeHeader {
  ...
  val jms2flowMessage: FlowHeaderConfig => JmsSettings => Message => Try[FlowMessage] = headerConfig =>
    settings =>
      msg =>
        Try {
          ...
          val props: Map[String, MsgProperty] = {
            ...
            val corrIdMap: Map[String, MsgProperty] =
              Option(msg.getJMSCorrelationID())
                .map { s =>
                  Map(
            ...
            val props: Map[String, MsgProperty] = msg
              .getPropertyNames()
              .asScala
              .map { name =>
            ...
            val replyToMap: Map[String, MsgProperty] =
              Option(msg.getJMSReplyTo())
                .map(d => replyToHeader(prefix) -> MsgProperty(JmsDestination.create(d).unwrap.asString).unwrap)
                .toMap
  ...
  val envelope2jms: (JmsProducerSettings, Session, FlowEnvelope) => Try[JmsSendParameter] =
    (settings, session, flowEnv) =>
      Try {
