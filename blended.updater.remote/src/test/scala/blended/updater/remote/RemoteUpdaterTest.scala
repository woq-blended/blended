...
import blended.updater.config.{Mapper => _, _}
...
class RemoteUpdaterTest extends LoggingFreeSpec with TestFile {
  ...
  def withEmptyRemoteUpdate(transient: Boolean)(f: TestContext => Unit): Unit = {
    transient match {
      case true =>
        val ru = new RemoteUpdater(new TransientRuntimeConfigPersistor(), new TransientContainerStatePersistor())
        f(TestContext(ru))
      case false =>
        withTestDir() { dir =>
    ...
    case (name, transient) =>
      s"When persistence is ${name}" - {
        ...
        "adding a stage action" in {
          withEmptyRemoteUpdate(transient) { ctx =>
            val action1 = StageProfile(UUID.randomUUID().toString(), "test", "1")
            ctx.remoteUpdater.addAction("1", action1)
            assert(ctx.remoteUpdater.getContainerActions("1") === Seq(action1))
          }
        }
        ...
        "adding a second add runtime config action" in {
          withEmptyRemoteUpdate(transient) { ctx =>
            val ru = new RemoteUpdater(
              new TransientRuntimeConfigPersistor(),
              new TransientContainerStatePersistor()
            )
            val action1 = AddRuntimeConfig(
              UUID.randomUUID().toString(),
              Profile(
        ...
        "adding a second stage action" in {
          withEmptyRemoteUpdate(transient) { ctx =>
            ...
            ctx.remoteUpdater.addAction("1", action1)
            assert(ctx.remoteUpdater.getContainerActions("1") === Seq(action1))
            val action2 = StageProfile(UUID.randomUUID().toString(), "test", "2")
        ...
        "adding a update action" in {
          withEmptyRemoteUpdate(transient) { ctx =>
            val action1 = ActivateProfile(UUID.randomUUID().toString(), "test", "1")
            ctx.remoteUpdater.addAction("1", action1)
            assert(ctx.remoteUpdater.getContainerActions("1") === Seq(action1))
          }
        }
        ...
        "adding a second update action" in {
          withEmptyRemoteUpdate(transient) { ctx =>
            val action1 = ActivateProfile(UUID.randomUUID().toString(), "test", "1")
            ctx.remoteUpdater.addAction("1", action1)
            val action2 = ActivateProfile(UUID.randomUUID().toString(), "test", "2")
            ctx.remoteUpdater.addAction("1", action2)
            assert(ctx.remoteUpdater.getContainerActions("1") === Seq(action1, action2))
          }
        }
        ...
        "add a runtime config and stage as new profile" in {
          withEmptyRemoteUpdate(transient) { ctx =>
            val conId = "1"
...
//              val state =
//                p.findByExample(PersistentContainerStatePersistor.pClassName, Map("containerId" -> conId).asJava)
//              assert(state.size === 1)
//              assert(Mapper.unmapContainerState(state.head).get.outstandingActions.size === 1)
//            }
