...
trait Mapper extends BaseMapper {
  ...
  def mapContainerState(containerState: ContainerState): java.util.Map[String, AnyRef] = {
    Map[String, AnyRef](
      ...
      "profiles" -> containerState.profiles.map(p => mapProfileRef(p)).asJava,
  ...
  def unmapContainerState(map: AnyRef): Try[ContainerState] = Try {
    ...
    ContainerState(
      ...
      outstandingActions = p("outstandingActions")
        .asInstanceOf[java.util.Collection[AnyRef]]
        .asScala
        .toList
        .map(a => unmapUpdateAction(a).get),
      profiles =
        p("profiles").asInstanceOf[java.util.Collection[AnyRef]].asScala.toList.map(p => unmapProfileRef(p).get),
