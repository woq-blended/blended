...
trait DistModule extends CoreCoursierModule {
  ...
  def expandedFilteredSources: Target[PathRef] =
    T {
      val dest = T.ctx().dest
  ...
  def transitiveLibModules: Seq[PublishModule] =
    libModules.flatMap(_.transitiveModuleDeps).collect { case m: PublishModule => m }.distinct

  /**
   * The transitive ivy dependencies of this module and all it's upstream modules
   */
  def transitiveLibIvyDeps: T[Agg[Dep]] =
    T {
  ...
  def resolvedLibs: Target[PathRef] =
    T {
      val dest = T.dest
      val libs = Target.traverse(transitiveLibModules)(m =>
        T.task {
...
class BlendedCross(crossScalaVersion: String) extends GenIdeaModule { blended =>
  ...
  trait CoreModule extends BlendedBaseModule with BlendedOsgiModule with CoreCoursierModule with CorePublishModule {
  ...
  object akka extends CoreModule {
    ...
    override def essentialImportPackage: Seq[String] =
      super.essentialImportPackage ++ Seq(
    ...
    object bridge extends CoreModule {
      ...
      override def testGroups: Map[String, Set[String]] =
        Map(
          ...
          "TransactionSendFailedRejectBridgeSpec" -> Set(
            "blended.jms.bridge.internal.TransactionSendFailedRejectBridgeSpec"
          ),
          "TransactionSendFailedRetryBridgeSpec" -> Set(
            "blended.jms.bridge.internal.TransactionSendFailedRetryBridgeSpec"
          ),
  ...
  object launcher extends CoreModule {
    ...
    object dist extends DistModule with CoreCoursierModule {
      ...
      override def filterProperties: Target[Map[String, String]] =
        T {
          Map(
      ...
      override def libModules: Seq[PublishModule] =
        Seq(
          blended.domino,
          blended.launcher,
          blended.util.logging,
    ...
    object test extends CoreTests {
      ...
      def resolvedOsgiFrameworks =
        T {
          Target
            .traverse(osgiFrameworkIvyDeps.toSeq) {
              case (name, dep) =>
          ...
          super.generatedSources() ++ {
            ...
            val body =
              s"""
                 ...
                 |  val frameworks: Map[String, String] = ${resolvedOsgiFrameworks()
                .map { case (name, file) => s""""${name}" -> "${file.path}"""" }
                .mkString("Map(\n    ", ",\n    ", "\n  )")}
                 |}
                 |
                 |object JvmLauncherTest {
                 |  val classpath: Seq[String] = ${blended.launcher.scoverage
                .runClasspath()
                .map(ref => s""""${ref.path}"""")
                .mkString("Seq(\n    ", ",\n    ", "\n  )")}
                 |}
  ...
  object prickle extends CoreJvmModule {
    ...
    override def essentialImportPackage =
      super.essentialImportPackage ++ Seq(
        "prickle",
        "microjson"
      )
    ...
    override def exportContents: T[Seq[String]] =
      T {
        Seq(
  ...
  object security extends CoreJvmModule {
    ...
    object scep extends CoreModule {
      ...
      object standalone extends BlendedBaseModule with CoreCoursierModule with CorePublishModule { scepclient =>
        ...
        override def manifest: T[JarManifest] =
          T {

            val mDeps: Seq[String] = Target
              .traverse(transitiveLibModules)(m =>
                T.task {
        ...
        object dist extends DistModule with CoreCoursierModule {
          ...
          override def resolvedLibs: Target[PathRef] =
            T {
              val dest = T.dest
  ...
  object itestsupport extends CoreModule {
    ...
    object test extends CoreTests {
      ...
      override def forkArgs =
        T {
          val jolokiaAgent: PathRef = compileClasspath()
            .filter { r =>
  ...
  object features extends BlendedFeatureJar with CorePublishModule with CoreCoursierModule {
    ...
    val coreDep: String => String => Dep = version =>
      name =>
        Dep(
          org = deps.blendedOrg,
          name = name,
      ...
      T {
        Feature(
          ...
          bundles = Seq(
            ...
            FeatureBundle(deps.jettyUtilAjax),
    ...
    override def features(): define.Command[Seq[Feature]] =
      T.command {
        Seq(
          activemq(),
          akkaHttpBase(),
