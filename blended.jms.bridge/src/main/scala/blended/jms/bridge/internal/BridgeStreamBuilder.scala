...
class BridgeStreamBuilder(
  ...
  protected val outId =
    s"${bridgeCfg.toCf.vendor}:${bridgeCfg.toCf.provider}:${bridgeCfg.toDest.map(_.asString).getOrElse("out")}"
  ...
  private class BridgeDestinationResolver(settings: JmsProducerSettings) extends MessageDestinationResolver(settings) {
    override def destination(env: FlowEnvelope): Try[JmsDestination] = {
      ...
      val dest: Try[JmsDestination] = (
        env.header[String](headerConfig.headerBridgeVendor),
        env.header[String](headerConfig.headerBridgeProvider)
      ) match {
        case (Some(v), Some(p)) =>
        ...
        case _ =>
          val result = super.destination(env)
          bridgeLogger.debug(
            s"Routing message to [${settings.connectionFactory.vendor}:${settings.connectionFactory.provider}][$result]"
          )
          result
      }
      ...
      dest match {
        ...
        case Failure(t) =>
          ...
          bridgeCfg.outboundAlternates.find { s => env.header[String](s).isDefined } match {
            case None =>
              log.warn(
                s"Failed to resolve alternative destination for [$transId] from [${bridgeCfg.outboundAlternates.mkString(",")}]"
              )
              throw t
            case Some(d) =>
  ...
  protected val retryCount: FlowEnvelope => Long = env =>
    env.header[Long](bridgeCfg.headerCfg.headerRetryCount).getOrElse(0L)
  ...
  protected def jmsSource: Source[FlowEnvelope, NotUsed] = {
    ...
    val src: Source[FlowEnvelope, NotUsed] = {
      val result: Source[FlowEnvelope, NotUsed] = Source
        .fromGraph(
          new JmsConsumerStage(
      ...
      transShard match {
        case None => result
        case Some(shard) =>
          result.via(Flow.fromFunction[FlowEnvelope, FlowEnvelope] { env =>
          ...
          logEnvelope(s"Forwarding to retry [$d]")
            .via(
              Flow.fromFunction[FlowEnvelope, FlowEnvelope] { env =>
                env
                  .withHeader(bridgeCfg.headerCfg.headerRetryDestination, JmsDestination.asString(bridgeCfg.fromDest))
                  .get
              }
            )
            .via(
  ...
  protected[bridge] val trackFilter: Graph[FanOutShape2[FlowEnvelope, FlowEnvelope, FlowEnvelope], NotUsed] =
    FlowProcessor.partition[FlowEnvelope] { env =>
  ...
  val stream: Source[FlowEnvelope, NotUsed] = {
    ...
    val clearRetrying: Graph[FlowShape[FlowEnvelope, FlowEnvelope], NotUsed] =
      FlowProcessor.fromFunction("clearRetrying", envLogger) { env =>
        Try {
  ...
  bridgeLogger.debug(
    s"Starting bridge stream with config [inbound=${bridgeCfg.inbound},trackTransaction=${bridgeCfg.trackTransaction}]"
  )
}
