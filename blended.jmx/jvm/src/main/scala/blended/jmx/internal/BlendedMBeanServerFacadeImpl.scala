...
class BlendedMBeanServerFacadeImpl(svr : MBeanServer) extends BlendedMBeanServerFacade {
  ...
  override def mbeanNames(objName : Option[JmxObjectName]): Try[List[JmxObjectName]] = Try {
    ...
    // scalastyle:off null
    svr.queryNames(objName.map(_.objectNamePattern).orNull, null).forEach(n => names.append(n))
    // scalastyle:on null
  ...
  def mbeanInfo(objName : JmxObjectName) : Try[JmxBeanInfo] = Try {
    ...
    val info : MBeanInfo = svr.getMBeanInfo(objName.objectName)
    ...
    val attrs : List[Attribute] = svr.getAttributes(objName.objectName, attrNames).asList().asScala.toList
