...
import java.nio.file.attribute.{GroupPrincipal, UserPrincipal}
import java.util
  ...
  override def login(): Boolean = {
    ...
    cbHandler match {
      ...
      case Some(cbh) => try {
        ...
        if (name == "andreas" && pwd == "test") {
          succeeded = true

          subject.foreach{ s =>
            s.getPrincipals().add(new UserPrincipal {
              override def getName: String = name
            })
            s.getPrincipals().add(new GroupPrincipal {
              override def getName: String = "hawtio"
            })
          }
          succeeded
        } else {
