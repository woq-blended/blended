...
class MgmtRestActivator extends DominoActivator with ActorSystemWatching {
  ...
  whenBundleActive {
    whenServicesPresent[PersistenceService, BlendedPermissionManager] {
      (persistenceService, mgr) =>
        ...
        val collectorService = new CollectorServiceImpl(remoteContainerStatePersistor, mgr, version)
