...
import scala.reflect.runtime.universe._
  ...
  package object protocol {
    ...
    implicit def bool2Property(b: Boolean)  : PersistenceProperty[Boolean] = PersistenceProperty(b)
    implicit def byte2Property(b: Byte)     : PersistenceProperty[Byte]    = PersistenceProperty(b)
    implicit def short2Property(s: Short)   : PersistenceProperty[Short]   = PersistenceProperty(s)
    implicit def int2Property(i: Int)       : PersistenceProperty[Int]     = PersistenceProperty(i)
    implicit def long2Property(l: Long)     : PersistenceProperty[Long]    = PersistenceProperty(l)
    implicit def float2Property(f: Float)   : PersistenceProperty[Float]   = PersistenceProperty(f)
    implicit def double2Property(d: Double) : PersistenceProperty[Double]  = PersistenceProperty(d)
    implicit def char2Property(c: Char)     : PersistenceProperty[Char]    = PersistenceProperty(c)
    implicit def string2Property(s: String) : PersistenceProperty[String]  = PersistenceProperty(s)
...
package protocol {
  ...
  abstract class DataObject(uuid : String) {
    ...
    def persistenceProperties : PersistenceProperties = (persistenceType, Map(DataObject.PROP_UUID -> uuid))
  ...
  case class CreateObjectFromProperties(props: protocol.PersistenceProperties)
