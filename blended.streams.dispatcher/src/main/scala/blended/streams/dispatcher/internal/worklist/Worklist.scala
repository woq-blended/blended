...
case class WorklistStarted(worklist : Worklist, timeout: FiniteDuration = 100.millis) extends WorklistEvent
...
case class WorklistManager(
  inEvents : Source[WorklistEvent, _],
  outEvents : Sink[WorklistEvent, _]
) extends GraphStage[FlowShape[WorklistEvent, WorklistEvent]] {

  private val in = Inlet[WorklistEvent]("Worklist.in")
  private val out = Outlet[WorklistEvent]("Worklist.out")
  ...
  override def createLogic(inheritedAttributes: Attributes): GraphStageLogic = new TimerGraphStageLogic(shape) {
    ...
    private val log = Logger[WorklistManager]
