...
class PasswordHasherSpec extends LoggingFreeSpec with Matchers with ScalaCheckPropertyChecks {
  ...
  "The password hasher should" - {
    ...
    "yield a different string for different passwords" in {
      ...
      forAll(genStrings, genStrings) { (p1: String, p2: String) =>
    ...
    "yield a different string for different salts and the same password" in {
      ...
      forAll(genStrings, genStrings) { (s1: String, s2: String) =>
