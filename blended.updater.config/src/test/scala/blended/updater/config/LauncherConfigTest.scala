...
class LauncherConfigTest extends AnyFreeSpec {
  ...
  "Minimal config" - {
    ...
    "read() -> toConfig() -> read() must result in same config" in {
      ...
      val toCfg = LauncherConfig.toConfig(config)
      val config2 = LauncherConfig.read(toCfg)
  ...
  "Complex config" - {
    ...
    "with prefix overlaying key in system settings" in {
      ...
      val a = LauncherConfig.read(ConfigFactory.parseString(config))
  ...
  "Config with placeholders" - {
    ...
    "read() -> toConfig must translate escaped placeholders" in {
      val config = """
       ...
       |""".stripMargin
      val a = LauncherConfig.read(ConfigFactory.parseString(config))
      val b = LauncherConfig.read(LauncherConfig.toConfig(a))
