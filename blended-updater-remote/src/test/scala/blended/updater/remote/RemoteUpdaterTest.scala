...
class RemoteUpdaterTest extends FreeSpec {
  ...
  "initial empty state" in {
    val ru = new RemoteUpdater with InMemoryPersistor {}
  ...
  "remove a stage action if container info reports already staged" in {
    ...
    ru.addAction("1", action1)
    assert(ru.getContainerActions("1") === Seq(action1))
    ru.removeResolvedActions(ContainerInfo("1", Map(), immutable.Seq(ServiceInfo("/blended.updater", System.currentTimeMillis(), 100000L, Map("profiles.valid" -> "test-1")))))
    assert(ru.getContainerActions("1") === Seq())
  }
  ...
  "remove an activation action if container info reports already activated" in {
    ...
    ru.addAction("1", action1)
    assert(ru.getContainerActions("1") === Seq(action1))
    ru.removeResolvedActions(ContainerInfo("1", Map(), immutable.Seq(ServiceInfo("/blended.updater", System.currentTimeMillis(), 100000L, Map("profile.active" -> "test-1")))))
    assert(ru.getContainerActions("1") === Seq())
  }
